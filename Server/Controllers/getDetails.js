...
const nearWorker = async (req, res) => {
  const { location } = req.params
  let loc = location.split(',')
  loc[0] = Number(loc[0])
  loc[1] = Number(loc[1])

  const sortedWorker = await workermodel.find().lean();
  const workersWithDistance = sortedWorker.map(work => ({
    ...work,
    distance: Math.sqrt(
      Math.pow(work.location[0] - loc[0], 2) +
      Math.pow(work.location[1] - loc[1], 2)
  ...
  // console.log(workersWithDistance)
...
module.exports = { getDetails, getRole, getHistory, addWorker, getComments, addComments, getJobs, addJob, addComplete, addRatings, getWorker, nearWorker }